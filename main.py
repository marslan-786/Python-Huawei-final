...
async def start_bot():
    ...
    if not BOT_RUNNING:
        ...
        if os.path.exists(NUMBERS_FILE):
            ...
            log_msg(f"🔥 Launching {CONCURRENT_WORKERS} Separate Browsers...")
...
async def worker_process(worker_id):
    ...
    # 🌟 Staggered Start: Prevent CPU/Net choke by waiting a bit
    await asyncio.sleep(worker_id * 1.5) 
    
    # Keep the Playwright engine alive, but launch/close browsers per number
    async with async_playwright() as p:
        while BOT_RUNNING:
            ...
            try:
                # 🔥 Process in a FRESH Browser Instance
                await process_single_number(p, phone_number)
...
# --- SINGLE NUMBER LOGIC (Fresh Browser) ---
async def process_single_number(playwright_instance, phone_number):
    proxy_cfg = get_proxy()
    ...
    try:
        ...
        log_msg(f"Processing: {phone_number} | Proxy: {proxy_cfg['server'] if proxy_cfg else 'Direct'}")

        # Navigate with Timeout Safety
        try:
            await page.goto(BASE_URL, timeout=30000)
        except Exception:
            log_msg(f"Error: {phone_number} - Network Timeout/Proxy Failed")
        ...
        try:
            ...
            log_msg(f"Error: {phone_number} - Page Load Failed")
            await browser.close(); return
